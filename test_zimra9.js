...
// Try WITH /api/v7.2 prefix (matching Python base URL)
const baseUrl = `${process.env.FDMS_BASE_URL}/api/v7.2`;
...
const payload = {
  ...
  receipt: {
    ...
    receiptCounter: 1,
    receiptGlobalNo: 1,
    invoiceNo: "INV-TEST-010",
    receiptDate: "2026-05-21T14:10:00",
...
async function testPayload() {
  console.log('=== Testing with /api/v7.2 prefix ===');
  console.log('URL:', `${baseUrl}/Device/v1/${DEVICE_ID}/SubmitReceipt`);
  
  try {
    const response = await axios.post(
      `${baseUrl}/Device/v1/${DEVICE_ID}/SubmitReceipt`,
      payload,
      {
        headers: {
